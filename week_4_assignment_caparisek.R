...
############## pull out 1st column & column 5-8 (only 1st 400 rows)
surveyhmwk
str(surveyhmwk)

surveys_subset<-surveyhmwk[c(1:400),c(1,5:8)]
str(surveys_subset)


#4) CHALLENGE: Select all rows that have a hindfoot_length greater than 32, save these in a new data.frame named surveys_long_feet, then plot its hindfoot_length values as a histogram using the hist function. Change the column hindfoot_lengths into a character vector.Plot the hindfoot_lengths in a histogram (if this doesn’t work, just leave it, and think about it during Part II of the assignment, wink wink).Commit & Push the R script to GitHub - check your repository on GitHub and make sure your script is there.
